...
addLayer("s", {
    ...
    upgrades: {
        ...
        15: {
            title: "Science Boost",
            description: "Gain scales with your Science Points.",
            cost: new Decimal(5000),
            effect() { return player.p.points.add(1).pow(0.5) },
            effectDisplay() { return "x" + format(upgradeEffect(this.layer, this.id)) }
        },
    ...
    gainMult() {
        let mult = new Decimal(1)
        for (let id = 11; id <= 20; id++) {
            if (hasUpgrade('s', id)) mult = mult.times(upgradeEffect('s', id))
        }
        return mult
    },
    ...
    row: 0,
    hotkeys: [
        {key: "s", description: "S: Reset for Science", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
    ],
    layerShown(){ return true }
})
